...
然后运行下面的安装脚本。它会自动探测当前项目目录、当前用户家目录，并使用项目内 `.venv/bin/python` 生成用户级 `systemd` 服务：
...
- 使用项目内 `.venv/bin/python` 作为服务解释器
- 自动执行 `systemctl --user daemon-reload`
- 自动执行 `systemctl --user enable --now whut-wlan.service`
...
```shell
systemctl --user status whut-wlan.service
```
...
它会停止并移除 `whut-wlan.service`，但默认保留 `whut-wlan.env`，方便后续重新安装。

手动关闭并取消开机自启：

```shell
systemctl --user disable --now whut-wlan.service
```
